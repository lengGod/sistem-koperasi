<x-app-layout>
    ...
    <section class="mb-8 flex flex-col justify-between gap-4 md:flex-row md:items-center">
        <div>
            ...
            <p class="mt-1 text-sm text-outline">Pantau anggota, simpanan, pinjaman, dan angsuran koperasi dari satu
                layar.</p>
        </div>

        <div class="flex flex-wrap items-center gap-2">
            <a href="{{ route('members.create') }}"
                class="inline-flex items-center gap-2 rounded-xl border border-outline-variant bg-surface-container-lowest px-4 py-2 text-sm font-bold text-primary shadow-sm transition hover:bg-primary-fixed">
    ...
    <section class="mb-8 grid grid-cols-1 gap-5 sm:grid-cols-2 xl:grid-cols-4">
        @foreach ([['label' => 'Total Anggota', 'value' => number_format($totalMembers), 'icon' => 'group', 'tone' => 'bg-primary-fixed text-primary', 'note' => 'Anggota terdaftar'], ['label' => 'Total Simpanan', 'value' => 'Rp ' . number_format($totalSavings, 0, ',', '.'), 'icon' => 'account_balance', 'tone' => 'bg-secondary-fixed text-secondary', 'note' => 'Akumulasi saldo dari data transaksi'], ['label' => 'Pinjaman Aktif', 'value' => number_format($activeLoans), 'icon' => 'payments', 'tone' => 'bg-tertiary-fixed text-tertiary', 'note' => 'Pinjaman berjalan'], ['label' => 'Angsuran Perlu Proses', 'value' => number_format($dueInstallments), 'icon' => 'event_repeat', 'tone' => 'bg-surface-container-high text-on-surface', 'note' => 'Menunggu, Dibayar, Terlambat']] as $card)
            <article class="dashboard-card rounded-3xl border border-white/70 bg-surface-container-lowest p-6">
                <div class="mb-5 flex items-start justify-between">
                    ...
                    <span
                        class="rounded-full bg-surface-container-low px-2.5 py-1 text-[11px] font-extrabold text-on-surface-variant">Laporan</span>
                </div>
    ...
    <section class="mb-8 grid grid-cols-1 gap-5 md:grid-cols-3">
        <article class="dashboard-card rounded-3xl bg-surface-container-lowest p-6">
            <div class="mb-4 flex items-start justify-between">
                ...
                <span
                    class="rounded-full bg-surface-container-low px-2.5 py-1 text-[11px] font-extrabold text-on-surface-variant">{{ $reportDate->locale(app()->getLocale())->translatedFormat('M Y') }}</span>
            </div>
            <p class="text-xs font-bold uppercase tracking-[0.16em] text-outline">Simpanan Masuk Periode</p>
            <h3 class="mt-1 text-3xl font-extrabold tracking-tight text-on-surface">Rp
                {{ number_format($totalSavingsThisMonth, 0, ',', '.') }}</h3>
        ...
        <article class="dashboard-card rounded-3xl bg-surface-container-lowest p-6">
            <div class="mb-4 flex items-start justify-between">
                ...
                <span
                    class="rounded-full bg-surface-container-low px-2.5 py-1 text-[11px] font-extrabold text-on-surface-variant">Saldo
                    berjalan</span>
            </div>
            <p class="text-xs font-bold uppercase tracking-[0.16em] text-outline">Saldo Pinjaman Aktif</p>
            <h3 class="mt-1 text-3xl font-extrabold tracking-tight text-on-surface">Rp
                {{ number_format($activeLoanBalance, 0, ',', '.') }}</h3>
        ...
        <article class="dashboard-card rounded-3xl bg-surface-container-lowest p-6">
            <div class="mb-4 flex items-start justify-between">
                ...
                <span
                    class="rounded-full bg-surface-container-low px-2.5 py-1 text-[11px] font-extrabold text-on-surface-variant">Status
                    angsuran</span>
            </div>
            <p class="text-xs font-bold uppercase tracking-[0.16em] text-outline">Angsuran Lunas / Terlambat</p>
            <h3 class="mt-1 text-3xl font-extrabold tracking-tight text-on-surface">
                {{ number_format($paidInstallments) }} / {{ number_format($overdueInstallments) }}</h3>
    ...
    <section class="grid grid-cols-1 gap-6 xl:grid-cols-12">
        <article class="dashboard-card rounded-3xl bg-surface-container-lowest p-6 xl:col-span-8">
            <div class="mb-8 flex flex-col justify-between gap-3 sm:flex-row sm:items-center">
                <div>
                    ...
                    <p class="text-sm text-outline">Saldo simpanan berjalan mengikuti akumulasi data seeder dan
                        transaksi tersimpan.</p>
                </div>
            ...
            <div class="rounded-2xl bg-surface-container-low p-5">
                <div class="flex h-72 items-end gap-3 sm:gap-5">
                    @foreach ($chartBars as $bar)
                        @php
                            ...
                            $installmentsHeight =
                                $bar['installments'] > 0 ? max(4, ($bar['installments'] / $maxTotal) * 100) : 0;
                        @endphp
                        <div class="group flex h-full min-w-0 flex-1 flex-col items-center justify-end gap-3">
                            <div class="flex min-h-0 w-full flex-1 items-end gap-1">
                                <div class="relative flex-1 rounded-t-xl bg-primary-container/35 transition group-hover:bg-primary-container"
                                    style="height: {{ $savingsHeight }}%">
                                    <span
                                        class="absolute -top-8 left-1/2 hidden -translate-x-1/2 whitespace-nowrap rounded-lg bg-on-surface px-2 py-1 text-[10px] font-bold text-inverse-on-surface group-hover:block">
                                        Rp {{ number_format($bar['savings'], 0, ',', '.') }}
                                    </span>
                                </div>
                                <div class="relative flex-1 rounded-t-xl bg-tertiary-fixed/80 transition group-hover:bg-tertiary"
                                    style="height: {{ $installmentsHeight }}%">
                                    <span
                                        class="absolute -top-8 left-1/2 hidden -translate-x-1/2 whitespace-nowrap rounded-lg bg-on-surface px-2 py-1 text-[10px] font-bold text-inverse-on-surface group-hover:block">
                            ...
                            <span
                                class="text-center text-xs font-bold leading-tight text-outline">{{ $bar['month'] }}</span>
                        </div>
                    @endforeach
                </div>
                ...
                <div class="mt-5 flex flex-wrap gap-4 text-xs font-bold text-on-surface-variant">
                    <span class="inline-flex items-center gap-2"><span
                            class="h-2.5 w-2.5 rounded-full bg-primary-container"></span>Akumulasi Simpanan</span>
                    <span class="inline-flex items-center gap-2"><span
                            class="h-2.5 w-2.5 rounded-full bg-tertiary"></span>Angsuran</span>
                </div>
            </div>
        </article>
        ...
        <article class="dashboard-card rounded-3xl bg-surface-container-lowest p-6 xl:col-span-4">
            <div class="mb-6 flex items-center justify-between">
                ...
                <a href="{{ route('savings.index') }}" class="text-sm font-bold text-primary hover:underline">Lihat
                    Semua</a>
            </div>
                    ...
                    <div class="flex gap-4">
                        ...
                        <div class="min-w-0">
                            <p class="text-sm leading-5 text-on-surface">
                                <strong>{{ $saving->member?->name ?? '-' }}</strong>
                                {{ $saving->transaction_type === 'deposit' ? 'menyetor' : 'menarik' }} Rp
                                {{ number_format((float) $saving->amount, 0, ',', '.') }}
                            </p>
                            <p class="mt-1 text-xs text-outline">
                                {{ optional($saving->transaction_date)->format('d M Y') }} &middot;
                                {{ $saving->savingsType?->name ?? '-' }}</p>
                        </div>
                    </div>
                @empty
            ...
            <div class="mt-8 border-t border-outline-variant pt-6">
                <div class="mb-4 flex items-center justify-between">
                    ...
                    <a href="{{ route('loans.index') }}" class="text-sm font-bold text-primary hover:underline">Lihat
                        Semua</a>
                </div>
                    ...
                    @forelse ($recentLoans as $loan)
                        <div class="flex gap-4">
                            ...
                            <div class="min-w-0">
                                ...
                                <p class="mt-1 text-xs text-outline">Rp
                                    {{ number_format((float) $loan->principal_amount, 0, ',', '.') }} &middot;
        ...
        <article class="dashboard-card overflow-hidden rounded-3xl bg-surface-container-lowest xl:col-span-12">
            <div
                class="flex flex-col justify-between gap-3 border-b border-outline-variant px-6 py-4 sm:flex-row sm:items-center">
                <div>
                        ...
                        @forelse ($upcomingInstallments as $installment)
                            <tr class="transition hover:bg-surface-container">
                                ...
                                <td class="px-6 py-4 text-on-surface-variant">
                                    {{ $installment->loan?->loan_number ?? '-' }}</td>
                                <td class="px-6 py-4 text-on-surface-variant">
                                    {{ optional($installment->due_date)->format('d M Y') }}</td>
                                <td class="px-6 py-4 font-semibold text-on-surface">Rp
                                    {{ number_format((float) $installment->amount, 0, ',', '.') }}</td>
                                <td class="px-6 py-4">
                                    @php
                                        $statusLabel =
                                            [
                                    ...
                                    @endphp
                                    <span class="{{ $statusClass }} rounded-full px-2.5 py-1 text-xs font-extrabold">
                                        {{ $statusLabel }}
                                    </span>
                                </td>
                                <td class="px-6 py-4 text-right">
                                    <a href="{{ route('installments.show', $installment) }}"
                                        class="font-bold text-primary hover:underline">Detail</a>
                                </td>
                            </tr>
                        @empty
                            <tr>
