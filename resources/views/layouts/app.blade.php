...
<head>
    ...
    <title>{{ isset($title) ? $title . ' | ' : '' }}Koperasi Digital - Admin</title>
...
<body class="bg-surface text-on-surface antialiased">
    @php
        $navItems = [
            ...
            ['label' => 'Anggota', 'icon' => 'group', 'href' => route('members.index'), 'active' => request()->routeIs('members.*')],
            ['label' => 'Jenis Simpanan', 'icon' => 'category', 'href' => route('savings-types.index'), 'active' => request()->routeIs('savings-types.*')],
            ['label' => 'Simpanan', 'icon' => 'account_balance_wallet', 'href' => route('savings.index'), 'active' => request()->routeIs('savings.*')],
            ['label' => 'Pinjaman', 'icon' => 'payments', 'href' => route('loans.index'), 'active' => request()->routeIs('loans.*')],
            ['label' => 'Angsuran', 'icon' => 'event_repeat', 'href' => route('installments.index'), 'active' => request()->routeIs('installments.*')],
        ];
    @endphp

    <div
        x-data="appShell"
        @open-confirm-modal.window="openConfirm($event.detail)"
    ...
    >
        ...
        <aside 
            ...
            class="fixed inset-y-0 left-0 z-50 w-64 border-r border-outline-variant bg-surface-container-lowest shadow-sm transition-transform duration-300 lg:translate-x-0 -translate-x-full lg:static"
        >
            <div class="flex h-full flex-col">
                <div class="flex items-center gap-4 px-6 py-6">
                    <div class="flex h-11 w-11 items-center justify-center rounded-xl bg-primary-container text-on-primary shadow-sm">
                    ...
                    <ul class="space-y-1">
                        @foreach ($navItems as $item)
                            <li>
                                ...
                                >
                                    <span class="material-symbols-outlined {{ $item['active'] ? 'icon-fill' : '' }}">{{ $item['icon'] }}</span>
        ...
        <div class="flex-1 min-w-0">
            <!-- Topbar -->
            <header class="sticky top-0 z-30 flex h-16 items-center justify-between border-b border-outline-variant bg-surface-container-lowest/95 px-6 backdrop-blur">
                ...
                <div class="flex items-center gap-4">
                    ...
                    <div class="relative" @click.outside="profileOpen = false">
                        <button @click="profileOpen = !profileOpen" type="button" class="flex items-center gap-2 rounded-full py-1 pl-1 pr-2 transition hover:bg-surface-container">
                            <span class="flex h-9 w-9 items-center justify-center rounded-full bg-primary text-sm font-bold text-on-primary">
                        ...
                        >
                            ...
                            <a href="{{ route('profile.edit') }}" class="flex items-center gap-2 px-4 py-3 text-sm font-semibold text-on-surface transition hover:bg-surface-container-low">
                                <span class="material-symbols-outlined text-[20px]">person</span>
                                Profil
                            </a>
                            <form action="{{ route('logout') }}" method="POST">
                                @csrf
                                <button type="submit" class="flex w-full items-center gap-2 px-4 py-3 text-left text-sm font-semibold text-error transition hover:bg-error-container">
            ...
            <main class="mx-auto max-w-[1440px] px-4 py-6 md:px-6 lg:px-8">
                {{ $slot }}
            </main>
            <!-- Toast Notification -->
            @php
            ...
            >
                <div class="flex items-start gap-3 rounded-2xl border p-4 shadow-xl
                    {{ $toastTone === 'error'
            ...
            >
                ...
                >
                    <div class="flex items-start gap-4">
                        ...
                        >
                            <span class="material-symbols-outlined" x-text="confirmTone === 'danger' ? 'warning' : 'info'"></span>
                        </div>
                        <div class="min-w-0 flex-1">
                    ...
                    <div class="mt-6 flex justify-end gap-2">
                        ...
                        <button
                            ...
                            class="rounded-xl px-4 py-2 text-sm font-bold shadow-sm transition"
