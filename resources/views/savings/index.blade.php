<x-app-layout>
    ...
    <form
        x-data="{ selected: [], savingsIds: @js($savingsIds) }"
        method="POST"
    ...
    >
        ...
        <div class="flex flex-col gap-3 border-b border-outline-variant px-6 py-4 sm:flex-row sm:items-center sm:justify-between">
            ...
            <div class="flex flex-wrap items-center gap-2">
                <button type="button" class="rounded-xl border border-outline-variant bg-surface-container-lowest px-4 py-2 text-sm font-bold text-on-surface-variant transition hover:bg-surface-container-low" @click="selected = selected.length === savingsIds.length ? [] : savingsIds.slice()">
                    Pilih Semua
                </button>
                ...
                <thead class="bg-surface-container-low text-xs font-extrabold uppercase tracking-[0.08em] text-on-surface-variant">
                    <tr>
                        <th class="w-14 px-6 py-4">
                            <input type="checkbox" class="rounded border-outline-variant text-primary focus:ring-primary" @change="selected = $event.target.checked ? savingsIds.slice() : []" :checked="selected.length === savingsIds.length && savingsIds.length > 0">
                        </th>
                ...
                <tbody class="divide-y divide-outline-variant">
                    @forelse ($savings as $saving)
                        <tr class="transition hover:bg-surface-container">
                            <td class="px-6 py-4">
                                <input type="checkbox" name="savings_ids[]" value="{{ $saving->id }}" class="rounded border-outline-variant text-primary focus:ring-primary" x-model="selected">
                            </td>
