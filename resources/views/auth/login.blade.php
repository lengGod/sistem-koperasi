...
<head>
    ...
    <title>Login | Koperasi Siger</title>
...
<body class="bg-background text-on-surface">
    <main class="login-canvas px-4 md:px-6">
        <div
            class="w-full max-w-[440px] bg-surface-container-lowest p-10 md:p-12 rounded-3xl shadow-md flex flex-col gap-8 transition-all duration-300">
            <div class="flex flex-col items-center text-center gap-2">
                <div class="">
                    <img src="{{ asset('logo.png') }}" alt="Logo Koperasi" class="h-16 w-16 shrink-0 object-contain">
                </div>
                <h1 class="text-[28px] leading-9 font-semibold tracking-tight text-on-surface">Koperasi Siger</h1>
                <p class="text-sm leading-5 text-outline">Kelola ekosistem koperasi Anda dengan presisi institusional.
                </p>
            </div>
            ...
            <form class="flex flex-col gap-4" method="POST" action="{{ route('login') }}">
                ...
                <div class="flex flex-col gap-1.5">
                    <label class="text-xs font-semibold tracking-wider uppercase text-on-surface-variant"
                        for="email">Username atau Email</label>
                    <div class="relative group">
                        <span
                            class="material-symbols-outlined absolute left-3 top-1/2 -translate-y-1/2 text-outline group-focus-within:text-primary transition-colors">person</span>
                        <input
                            ...
                            id="email" name="email" value="{{ old('email') }}" placeholder="admin@koperasi.id"
                ...
                <div class="flex flex-col gap-1.5">
                    <div class="flex justify-between items-center">
                        ...
                        @if (Route::has('password.request'))
                            <a class="text-xs font-semibold tracking-wider uppercase text-primary hover:underline transition-all"
                                href="{{ route('password.request') }}">Lupa kata sandi?</a>
                        @endif
                    </div>
                    <div class="relative group">
                        <span
                            class="material-symbols-outlined absolute left-3 top-1/2 -translate-y-1/2 text-outline group-focus-within:text-primary transition-colors">lock</span>
                        <input
                ...
                <div class="flex items-center gap-2 py-2">
                    ...
                    <label class="text-sm leading-5 text-on-surface-variant cursor-pointer" for="remember">Ingat
                        perangkat ini selama 30 hari</label>
                </div>
            ...
            <div class="pt-4 border-t border-outline-variant/30 flex flex-col gap-4 text-center">
                <p class="text-sm leading-5 text-on-surface-variant">
                    Butuh bantuan? <a class="text-primary font-semibold hover:underline" href="#">Hubungi
                        Dukungan</a>
                </p>
